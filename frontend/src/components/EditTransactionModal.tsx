...
const CATEGORIES = [
    ...
    'Kesehatan', 'Utilitas', 'Pendidikan', 'Investasi', 'Pendapatan', 'Lain-lain'
];
...
export default function EditTransactionModal({ isOpen, onClose, onSuccess, transaction }: EditTransactionModalProps) {
    ...
    useEffect(() => {
        if (transaction) {
            ...
            setFormData({
                merchant_deskripsi: transaction.merchant_deskripsi || '',
                nominal: transaction.nominal.toString(),
                tipe: transaction.tipe,
                kategori_otomatis: transaction.kategori_otomatis || 'Lain-lain',
                tanggal_waktu: localISOTime
            });
        }
    }, [transaction]);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden animate-in zoom-in-95 duration-200">
                <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                    <h3 className="text-xl font-bold text-slate-900 dark:text-white">Edit Transaksi</h3>
                    <button 
                        onClick={onClose}
